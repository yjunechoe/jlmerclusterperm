...
The CPA procedure identifies empirical clusters in a time series data and tests the significance of the observed cluster-mass statistics against **bootstrapped permutations** of the data. The bootstrapping procedure samples from the null, yielding a distribution of cluster-mass statistics emerging from "chance" (i.e., random shuffling of condition labels). The statistical significance of a cluster is the probability of observing a cluster-mass statistic as extreme as the cluster's given the simulated null distribution. 
...
The package [vignettes](https://yjunechoe.github.io/jlmerclusterperm/articles/) are roughly divided into two groups: **topics** and **case studies**. If you are a researcher with data ready for analysis, it is recommended to go through the case studies first and pluck out the relevant bits for your own hypothesis.

In the order of increasing complexity, the **case studies** are:

1) [Garrison et al. 2020](https://yjunechoe.github.io/jlmerclusterperm/articles/Garrison-et-al-2020.html), which introduces the package's interface to CPA and compares a wholesale vs. piecemeal approach to CPA. It also compares the use of linear vs. logistic regression for the calculation of timewise statistics.
...
The **topics** are not particular to one type of analysis and instead cover general package features:
...
- [Reproducibility](https://yjunechoe.github.io/jlmerclusterperm/articles/reproducibility.html): working with the Julia RNG to make CPA runs reproducible.

- [Asynchronous CPA](https://yjunechoe.github.io/jlmerclusterperm/articles/asynchronous-cpa.html): using the [future](https://future.futureverse.org/) package to run a CPA in a non-blocking background process.
...
- The JOSS paper for the permuco package ([Frossard & Renaud 2021](https://www.jstatsoft.org/article/view/v099i15)) and the references therein.

- [Benedikt V. Ehinger's 2016 blog post](https://benediktehinger.de/blog/science/statistics-cluster-permutation-test/), which includes a detailed graphical explainer for the CPA procedure.
