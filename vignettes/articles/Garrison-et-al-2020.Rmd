...
4) Using logistic regression over trial-level data of 1s and 0s
...
timewise_statistics_fig <- ggplot(mapping = aes(time, statistic)) +
  ...
  geom_line(
    aes(color = "aggregated"), linewidth = 1.5,
    data = tidy(empirical_statistics)
  ) +
  geom_line(
    aes(color = "trial-level"), linewidth = 1.5,
...
These appear to converge at a higher `nsim` for this data set. Here, we use `reset_rng_state()` to make them generate the same permutations and time the 1000-simulation runs:

```{r CPA_agg_1000}
system.time({
  reset_rng_state()
  CPA_agg_1000 <- clusterpermute(
    ts_data_agg_spec, threshold = 2,
    nsim = 1000, top_n = 1, progress = FALSE
  )
})
CPA_agg_1000$empirical_clusters
```
...
system.time({
  ...
  CPA_trial_1000 <- clusterpermute(
    ts_data_trial_spec, family = "binomial", threshold = 2.8,
    nsim = 1000, top_n = 1, progress = FALSE
  )
})
CPA_trial_1000$empirical_clusters
```
