...
If you wish to use other Julia packages not listed here like [Effects.jl](https://github.com/beacon-biosignals/Effects.jl), it is recommend to install them in the global library in a fresh Julia session as opposed to `Pkg.add()`-ing using the [JuliaConnectoR](https://github.com/stefan-m-lenz/JuliaConnectoR) interface, as that may pollute the `jlmerclusterperm` project environment (if you suspect that this has happened, just re-install the package).
