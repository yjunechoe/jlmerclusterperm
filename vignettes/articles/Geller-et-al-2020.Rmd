...
The data from the package (`cursive_agg_data`) looks slightly different from that used in the tutorial, so to ensure full reproducibility we use the [snapshot of the data](https://raw.githubusercontent.com/jgeller112/drjasongeller/main/content/blog/2020-07-10-CBPT/blog_data.csv) that the tutorial used:
...
The following reproduces the figure from the tutorial (it looks slightly different because [the data has been updated since](https://github.com/dmirman/gazer/commit/091d38f586640831dbfd87aae29965ecb0d2e253)):
...
Note that for within-participant predictors like `script`, the permutation algorithm will randomly swap the labels for condition between the trials from each subject. This preserves the temporal structure of the data (no swapping below the trial-level grouping) and subject-level groupings of the data (no swapping of trials across participants).
...
This returns the same cluster but now with a larger cluster-mass statistic, as expected (chi-squared is asymptotic to t^2).
...
We find the same clusters identified between 2000ms-2300ms for both "t" and "chisq", with the peaks more pronounced for "chisq". The differences between the two measures are inconsequential for this example, but may produce different results in other cases.

<div class="callout" style="font-size:0.9em">
The chi-squared statistic (which `jlmerclusterperm` computes via a likelihood ratio test) is often preferred for testing single parameters because it makes less assumptions and tend to be more robust ([glmmFAQ](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#model-diagnostics)). But because goodness-of-fit tests are at the level of a predictor in a model formula, "chisq" is less interpretatable for multi-level predictors (k-1 > 1). To tease apart the contribution of the individual levels of a predictor, it is recommended to use the `statistic = "t"` instead.
</div>

## D) Specifying random effects

The biggest missing component at this point is the subject random effects, which the original tutorial's anova model specified via the error term `Error(subject)`. There isn't a strict equivalent to this in linear regression, but specifying random intercepts by subject with `(1 | subject)` gets us very close:
...
At this point we may wonder whether the results change much if we used a maximal model with the random effects structure `(1 + script | subject)`. We first create a specification object with that formula:
...
Then compute the timewise statistics. We find one by-time model with singular fit which isn't too troubling:
...
timewise_fig_re +
  geom_line(
    aes(color = "chisq-re-max"),
    linewidth = 1.5,
    data = tidy(timewise_chisqs_re_max)
  )
```

This becomes clearer when we plot the _difference_ in timewise statistics between the two - they're practically zero:

```{r re-timewise-diff}
ggplot(tidy(timewise_chisqs_re - timewise_chisqs_re_max)) +
  geom_path(aes(x = time, y = statistic)) +
  scale_y_continuous(labels = function(x) sprintf("%f", x))
```
...
We demonstrate this in the following, which uses the maximal model. Notice how the results are identical to `re_CPA` but with a substantial slowdown:
...
Lastly, back in our figure of the data, we annotate the clusters detected with the threshold of 0.05 (chisq p-value) using the intercept-only model:
...
The last piece of the puzzle is the flipped sign of the effect. Whereas we detect a _negative_ cluster where the line for cursive is over the line for print, original tutorial detects a _positive_ cluster (output repeated below):
...
Or we can sum-code and set "print" to -1 and "cursive" to 1:
...
<div class="callout" style="font-size:0.9em">
Note that because CPA operates over the domain of the timewise statistics and not the effect size (i.e., the coefficient/beta), whether the difference between "print" and "cursive" was in unit of 1 (in treatment coding) or 2 (in contrast coding) has no bearing on the results.
</div>

To conclude, we re-run and time a 1000-simulation CPA using the intercept-only model with a treatment coding of `script` where  "print" as the reference level.
...
Finally, annotating just the significant cluster on the plot of the data:
