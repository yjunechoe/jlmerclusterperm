...
The chi-squared statistic (which `jlmerclusterperm` computes via a likelihood ratio test) is often preferred for testing single parameters because it makes less assumptions and tend to be more robust ([glmmFAQ](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html#model-diagnostics)). But because goodness-of-fit tests are at the level of a predictor in a model formula, "chisq" is less interpretable for multi-level predictors (k-1 > 1). To tease apart the contribution of the individual levels of a multi-level predictor, `statistic = "t"` may be more appropriate.
</div>

## D) Specifying random effects

The biggest missing component at this point is the subject random effects, which the original tutorial captures via the error term `Error(subject)`. There isn't a strict equivalent to this in regression models, but specifying random intercepts by subject with `(1 | subject)` gets us very close:
...
We also plot the timewise statistics from the random intercept models:
...
This becomes clearer when we plot the _difference_ in timewise statistics between the two - they're virtually identical:
...
<div class="callout" style="font-size:0.9em">
We run the above maximal mixed model CPA purely for demonstration purposes. The random effects structure is actually unidentifiable given the aggregated data, where at any point in time there are only 2 observations from each subject (the means at each condition).
</div>

Lastly, back in our figure of the data, we annotate the clusters detected with the threshold of 0.05 (chisq p-value) from the random intercept CPA:
...
<div class="callout" style="font-size:0.9em">
Because CPA operates over the domain of the timewise statistics (and not the effect size), whether the difference between "print" and "cursive" is expressed as a unit of 1 (in treatment coding) or 2 (in contrast coding) has no bearing on the calculation of the statistic.
</div>
