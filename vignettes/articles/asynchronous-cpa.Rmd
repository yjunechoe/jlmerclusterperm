...
The `{future}` package allows such asynchronous evaluation of R code. It's a big package implementing a complex topic and you can read more about the project on the [futureverse](https://www.futureverse.org/); we just show the bare minimum here.

For more advanced users, note that we're use parallelization _only_ for its asynchronous properties (non-blocking evaluation of R code in a background process). It is not recommended to start _multiple R processes_ with each running a CPA because the Julia session is already multithreaded (if you can spare more, set that in `options("jlmerclusterperm.nthreads")` before initializing).
...
3. Grab the `jlmerclusterperm` package environment to pass to the future

```{r future-setup}
library(future)
plan(multisession)
pkgenv <- jlmerclusterperm:::.jlmerclusterperm
```

The third step creates a variable called `pkgenv` that points to the internal package environment. Please treat this as a read-only object - it's unexported and not meant to be manipulated.
...
The template is as follows. First, you make the `pkgenv` variable (defined above) available for the future (via the `globals` argument). Then, inside the future expression ensure that the background process shares the same Julia session (via `list2env(...)`).

```{r eval = FALSE}
# Not run
f <- future(
  {
    list2env(as.list(pkgenv), jlmerclusterperm:::.jlmerclusterperm)
    ## < Your CPA Code Here > ##
  },
  globals = structure(TRUE, add = "pkgenv")
)
```
...
Once the future is created, it **immediately** starts evaluating the code in a background process. It is important that you **do not evaluate the future object directly**. Instead, you should query it with `future::resolved()` - it simply tells you whether the background evaluation has completed:
...
Lastly, although the R code here was just a crude progress alert using `Sys.sleep()`, you can freely evaluate _any_ R code **except other `jlmerclusterperm` functions that call Julia**. You can make plots, clean data, write up analyses, etc., but just don't run another CPA while a CPA is already running (because the Julia process is shared).
...
Once that's confirmed, we can use `value()` to collect the results from the future and assign it to a variable:

```{r CPA_async}
CPA_async <- value(f)
```

Calling `value()` also prints the messages encountered while evaluating the CPA code. We see this because we did not specify `progress = FALSE` in the `clusterpermute()` code passed to the future object. Critically, adding up the times from the message is similar to what we saw from the while loop (`r i/2` seconds).

The output of the asynchronous CPA (`CPA_async`) is identical to the initial CPA ran in the interactive sessionat the start (`CPA`) because both were ran with the RNG counter value of `123L`:
