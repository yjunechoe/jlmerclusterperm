...
The difference between these two runs is specifically in the **null distribution** of the cluster-mass statistics. Both 100-simulation samples are drawn share the same underlying (null) distribution, but the samples themselves are not identical to one another (they rarely are).
...
<div class="callout" style="font-size:0.9em">
The two colored lines represent two 100-simulation bootstrap permutations that differ in the initial RNG counter by a value of 1. The point is that the shape of the two lines look very different, which is unexpected under the assumption that having a similar counter value also means having many overlapping samples (i.e., permutations of the data) from the null.
</div>
